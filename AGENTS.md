...
Пока документ в статусе `Draft` (продукта ещё нет) — версия не увеличивается при каждой правке, остаётся `0.1`. Все изменения фиксируются только в таблице «История изменений» (дата, автор, что изменилось), без роста номера версии. Версия переходит в `1.0` только при реальном ревью/апруве и смене статуса на `Approved` (как это уже сделано в `01_Product_Vision.md`).
